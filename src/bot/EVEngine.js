class EVEngine {
  ...
  calculateAdjustedEV(modelProb, marketPrice, direction, costs = {}) {
    ...
    const spread = parseFloat(costs.spread);
    const slippage = parseFloat(costs.estimatedSlippage);
    const feeRateRaw = parseFloat(costs.takerFeeRate);
    const spreadCost = (Number.isFinite(spread) ? spread : 0) * 100;
    const slippageCost = (Number.isFinite(slippage) ? slippage : 0.005) * 100;
    const feeRate = Number.isFinite(feeRateRaw) ? feeRateRaw : 0.07;
    const outcomePrice = direction === 'NO' ? 1 - marketPrice : marketPrice;
    // Protocol fee per share: r*p*(1-p). Unlike the legacy flat 20bp
    // subtraction, this uses the price of the token actually being bought.
    const feeCost = feeRate * outcomePrice * (1 - outcomePrice) * 100;
