...
router.get('/', async (req, res) => {
  try {
    ...
    let sessionFilter = '';
    let params = [req.userId, limit, offset];
    
    if (req.query.session === 'current') {
      const active = await pool.query('SELECT id FROM trading_sessions WHERE user_id = $1 AND ended_at IS NULL ORDER BY started_at DESC LIMIT 1', [req.userId]);
      if (active.rows.length > 0) {
        sessionFilter = 'AND session_id = $4';
        params.push(active.rows[0].id);
      } else {
        sessionFilter = 'AND session_id = -1'; // No active session
      }
    }
    ...
    const countParams = req.query.session === 'current' && params.length === 4 ? [req.userId, params[3]] : [req.userId];
    const countFilter = req.query.session === 'current' && params.length === 4 ? 'AND session_id = $2' : (req.query.session === 'current' ? 'AND session_id = -1' : '');
...
router.get('/stats', async (req, res) => {
  try {
    ...
    if (req.query.session === 'current') {
      const active = await pool.query('SELECT id FROM trading_sessions WHERE user_id = $1 AND ended_at IS NULL ORDER BY started_at DESC LIMIT 1', [req.userId]);
      if (active.rows.length > 0) {
        sessionFilter = 'AND session_id = $2';
        params.push(active.rows[0].id);
      } else {
        sessionFilter = 'AND session_id = -1'; // No active session
      }
