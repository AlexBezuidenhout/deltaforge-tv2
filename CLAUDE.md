# Printer Blast — Claude Code Project Context

## What This Is
Full upgrade of Printer CLOB v2 into a **signal dashboard for manual scalp**.
Combines the original heuristic with a Φ-model (closed-form fair probability),
Wilder ATR/RSI/ADX/MACD/BB, dynamic realized volatility, slippage walker,
liquidity USD per side, oracle-lag monitor, and volume-spike detector — all in
an **ensemble** (mode B: two probability estimates blend into one `modelProb`).

The default philosophy matches **Printer CLOB v2 / fmrxr polybot-backend** (strict
pre-filters and EV floor), while keeping the **Blast stack**: Φ-model, ensemble,
indicators, slippage walk, depth USD, oracle lag. Auto-execute still works (paper
by default).

Stack: Node.js + Express + PostgreSQL + Polymarket CLOB SDK v5 + Binance WS/REST + Chainlink oracle.
Port: **3004** by default. DB is shared with Printer Mix unless `.env` overrides.

## What Changed vs Printer CLOB v2

New files in `src/bot/`:
- **`PhiModel.js`** — `P(UP) = Φ((btc − ref) / (σ · √(tte/300)))` with Abramowitz-Stegun CDF.
- **`BTCIndicators.js`** — Wilder ATR(5) / RSI(5) / ADX(14) with +DI/-DI, classical MACD(12,26,9), Bollinger Bands(20,2σ), annualised realised volatility, volume-spike ratio.
- **`SlippageEngine.js`** — walks the order book per side, returns `{ avgFillPrice, slipBps, fillable, sharesFilled }` for a given USD size.
- **`SignalEnsemble.js`** — mode B combiner. Blends Φ + heuristic by weight, flags AGREE / NEUTRAL / WEAK_DISAGREE / DISAGREE, returns a confidence multiplier (×1.10 on AGREE, ×0.70 on DISAGREE).

Extended files:
- **`BinanceFeed.js`** — REST poll of `/api/v3/klines?interval=1m&limit=60` every 30 s; `getRefBtcPriceAt(startSec)` looks up the 5 m kline open as Φ-anchor; `getDynamicSigma5min(20)` computes rolling σ from log-returns; `getATR / getRSI / getADX / getIndicators / getMACD / getBollinger / getVolumeSpike` are thin wrappers over `BTCIndicators`. Falls back across `api.binance.com → api1 → api2`.
- **`ChainlinkFeed.js`** — `getOracleLagMs(cexTimestampMs)` returns the gap between Chainlink `updatedAt` and the latest CEX tick; used as an info gate because Polymarket resolves on Chainlink.
- **`PolymarketFeed.js`** — order book now also exposes `bidLevels[]`, `askLevels[]`, `bestBidUsd`, `bestAskUsd` so `SlippageEngine` can walk it.
- **`GBMSignalEngine.js`** — full rewrite of the model-probability step:
  - window timing (`marketStartSec/EndSec/remaining`) computed *before* the model;
  - **scenario / btcFlat / neutralBlock** run as **hard skips** before Φ (same rules as v2 backend);
  - `btcPrecheck` skips the **entire market scan** when `|btcΔ60s| < min_btc_delta` (polybot parity);
  - Φ-model runs with dynamic σ (rolling 5 m), heuristic still runs in parallel;
  - `SignalEnsemble.combine(pPhi, pHeur)` produces the final `modelProb`;
  - depth floor defaults to **100 USDC** (`min_depth_usdc`); **fillProb** below `fillprob_floor` (default **0.25**) is a hard skip;
  - Gate 1 microstructure stays **informational** (same as v2);
  - new metrics on every signal: `phi`, `ensemble`, `indicators`, `macd`, `bollinger`, `volumeSpike`, `slippage`, `depthUsd`, `oracleLagMs`, `modelProbSource`.
- **`src/models/db.js`** — Blast-only columns plus `CREATE`/`ALTER` defaults aligned with polybot-backend; final `UPDATE bot_settings … WHERE true` applies the **strict migration** (EV floor, BTC deltas, chop override, depth/fill floors) while preserving Φ/ensemble via `COALESCE` where noted.
- **`src/index.js`** — CORS allowlist now auto-includes the current `PORT` and all common local dev ports (3000–3004, 5173) so same-origin requests from the bundled dashboard are never blocked.

## Boot-time settings backfill (each `initDB()`)

**Set-if-NULL only** (audit 2026-07-10): every assignment is `COALESCE(col, default)` —
a value the operator saved is NEVER overwritten at boot. The table below lists the
defaults applied when a column is NULL. (An earlier version reset some of these every
boot; that behavior was a bug and has been removed — do not reintroduce it.)
...
| `min_confidence` | **0.15** |
| `min_btc_delta` | **0.005 %** |
| `min_strong_btc_delta` | **0.050 %** |
| `snipe_timer_seconds` | **5** |
| `range_chop_gamma_override` | **0.010** |
| `phi_enabled` / `phi_sigma_5min` / `ensemble_phi_weight` | preserved via `COALESCE` if already set |
| `min_depth_usdc` | **100** (minimum enforced if null) |
| `fillprob_floor` | **0.25** |
| `slip_check_size_usd` | **5** |
| `oracle_lag_max_ms` | **5000** |
| `volume_spike_ratio` | **2.0** |
| `paper_trading` | `COALESCE(..., true)` |

Boundary-only books (`spread ≥ 90 %`) still return `SKIP` — no fillable price.

## Signal Pipeline (per tick)

1. **Pre-filters**: freshness, no-chase, **`btcPrecheck` (hard)** — may skip entire scan.
2. **Per market** (loop over discovered BTC 5 m markets):
   1. Price discovery (WS → CLOB → Gamma → cached).
   2. Window timing (`elapsed` / `remaining`).
   3. **Scenario filter (hard)**, **`btcFlat` (hard)**, **`neutralBlock` (hard)** — before Φ.
   4. Indicators snapshot (ATR/RSI/ADX/MACD/BB/volume spike).
   5. Φ-model with dynamic σ; heuristic `pHeur` in parallel.
   6. **Ensemble combine** → `modelProb`, `votes`, `confidenceMul`.
   7. Gate 1 microstructure (**informational**).
   8. Time gate (last 300 s), boundary-book guard (**hard**), depth (**hard**), fillProb (**hard**).
   9. EV with scenario multipliers → Gate 2 (**hard**).
  10. EV-decay filter.
  11. Gate 3 (BTC momentum direction).
  12. Confidence × `confidenceMul`; `min_confidence` (**hard**).
3. Output signal carries **all** intermediates: `phi`, `ensemble`, `indicators`, `slippage`, `depthUsd`, `oracleLagMs`, `volumeSpike`, `macd`, `bollinger`.

## What This Is NOT
- Not a guarantee of profit.
- No straddle / theta-decay mode.
- No hedge / maker-rebate mode (those need their own execution path).
- Pulse-style multi-timeframe scoring (10 s / 4 h / daily) is **not** ported — for 5 m binary it adds noise.

## Source Project (Unchanged — reference)

Original Polybot context below for reference. Where it conflicts with this Printer Blast section, this section overrides.
...
- Synthetic quote treated as executable book → WS/Gamma `mid ± 1¢` replaced the real CLOB object, allowing paper entry 0.52 against venue ask 0.96; fix: preserve side-specific `executionBooks` and re-run Gate 2 at depth/latency-adjusted ask
- Fee-model mismatch → EV used flat 20bp while paper P&L charged 2% of positive gains; fix: crypto taker curve `shares × 0.07 × p × (1-p)` at every taker match, zero maker rebate assumption
...
- Paper fill simulation: market-style buy walks recorded CLOB ask depth, sizes down to available liquidity, adds the configured latency tick, and rechecks executable Gate 2; no real book means no execution-honest evidence
...
- Node test suite: `node --test test/*.test.js`; analysis scripts under `scripts/`
