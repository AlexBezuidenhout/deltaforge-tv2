...
const FAMILY_PROFILES = Object.freeze({
  ...
  prediction: {
    label: 'Prediction-market calibration and bounded statistical residuals',
    persistence: 'Behavioral or operational calibration wedges may persist by price/category/time state, but broad directional forecasting is already a negative control.',
    capacityAndHalfLife: '$5–$50 per market for this bankroll; half-life ranges from seconds near expiry to the full contract duration.',
    requiredData: ['pre-trade executable quote', 'frozen model probability', 'book depth', 'current fees', 'terminal outcome', 'market/day identity'],
    dataReadiness: 'B — resolved markets and books exist, but historical discovery cohorts are heavily selected and some execution is synthetic.',
    executionPath: 'Start from the market quote, apply a frozen out-of-sample residual with uncertainty shrinkage, and cross only an executable ask below the conservative bound.',
    latencySensitivity: 'Usually 100 ms–minutes; only source-confirmed late-window residuals warrant faster profiles.',
    costModel: 'Current taker fee + walked depth + non-fill + latency decay + calibration/model uncertainty reserve.',
    legalTermsDependency: 'Exact resolution rule, source and market state; category-specific fee schedule must be versioned.',
    independentUnit: 'One resolved market, clustered by event/day and asset/category.',
    leakageSelectionRisks: 'Price-bucket mining, repeated signals per market, selecting winning assets and reusing discovery outcomes.',
    ...
    testTimeAndDisk: '14–30 days and at least 300 fresh markets; existing normalized facts are sufficient for initial falsification.',
    notStaleReason: 'Only a residual around the market prior is allowed; generic market-beating directional models are rejected.',
    sources: ['polymarketFees', 'polymarketBooks', 'polymarketResolution', 'overfitPaper'],
    baseScore: { mechanismStrength: 2, dataReadiness: 4, bankrollCapacity: 4, persistence: 2, dublinFit: 3, simplicity: 4, boundedRisk: 2, cheapFalsification: 5, independence: 5, postCostDollarOpportunity: 2 },
  },
  resolver: {
...
const FAMILY_SEEDS = Object.freeze({
  ...
  prediction: [
    seed('Q01', 'Market-prior calibration residual', 'Fit only the out-of-sample residual around the executable market quote and shrink it toward zero by its uncertainty.', 'Makers whose quote is well calibrated overall but conditionally biased in a stable state.', { decision: DECISIONS.CHEAP_FALSIFICATION, score: { mechanismStrength: 3, postCostDollarOpportunity: 3 } }),
    seed('Q02', 'Exact 0–20¢ longshot successor', 'Test whether the unchanged first-intent MAIN rule has a persistent underpricing residual only in the extreme longshot bucket.', 'Potential favorite–longshot bias or asymmetric retail demand.', { decision: DECISIONS.CONTINUE_FROZEN, existingEvidence: 'Fresh v19 successor: 1 fill, -$8.49 at doubled cost; 29 discovery fills are excluded.', score: { persistence: 2, postCostDollarOpportunity: 3 } }),
    seed('Q03', 'Category-specific favorite–longshot bias', 'Estimate a monotone calibration curve by pre-registered category and price region, trading only a lower confidence bound after fees.', 'Retail preference for lottery-like or near-certain payoffs.', { decision: DECISIONS.CHEAP_FALSIFICATION }),
    seed('Q04', 'Time-to-resolution hazard residual', 'Model the residual evolution of a bounded binary martingale by time remaining and quote state without importing GBM price dynamics.', 'Quote setters using coarse late-window risk buffers.', { decision: DECISIONS.CHEAP_FALSIFICATION, score: { mechanismStrength: 3 } }),
    seed('Q05', 'Source-confirmed quote underreaction', 'After a causally fresh authoritative public source change, estimate only the remaining quote residual and require a multi-tick dwell.', 'Temporarily stale prediction quotes.', { decision: DECISIONS.COLLECT_ONLY, score: { mechanismStrength: 4, persistence: 3 } }),
    seed('Q06', 'Ambiguity-conditioned risk premium', 'Test whether markets with objectively coded fallback/dispute states exhibit a stable excess return after duration and liquidity controls.', 'Capital avoiding ambiguous resolution risk.', { decision: DECISIONS.COLLECT_ONLY, score: { boundedRisk: 1 } }),
    seed('Q07', 'Absorbed-martingale local volatility', 'Estimate state-dependent quote variance on [0,1] with absorbing endpoints and test bounded transition probabilities, not Black–Scholes GBM.', 'Participants using misspecified short-horizon binary dynamics.', { decision: DECISIONS.CHEAP_FALSIFICATION, score: { mechanismStrength: 3, simplicity: 1 } }),
    seed('Q08', 'Winning-wallet imitation', 'Copy a historically profitable public wallet after observing its transactions.', 'No identified payer; copied fills arrive later and selection ignores hidden inventory/hedges.', { decision: DECISIONS.REJECTED_MECHANISM, existingEvidence: 'Wallet forensics can generate hypotheses but past wallet P&L is not a causal executable signal.', score: { mechanismStrength: 0, persistence: 0, boundedRisk: 0, postCostDollarOpportunity: 0 } }),
  ],
  resolver: [
