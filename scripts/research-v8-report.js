...
async function buildReport(pool, since = MANIFEST.evidence_started_at, now = new Date()) {
  ...
  const [{ rows: runtimeRows }, { rows: resultRows }, { rows: latencyRows }] =
    await Promise.all([
      pool.query(`
        ...
        SELECT r.strategy,r.cadence,r.market_types,r.evaluations,r.actions,
               r.errors,r.last_evaluated_at,r.last_action_at,r.updated_at,
      ...
      pool.query(`
        WITH raw AS (
          SELECT o.id,o.strategy,o.market_id,o.available_at,o.price,o.size,
                 o.features->>'group_id' group_id,
                 s.filled,s.fill_price,s.fill_size,s.pnl_1x,s.pnl_2x,
                 s.data_quality_grade,s.execution_fidelity_grade,s.fill_ts
        ...
        SELECT strategy,
               count(*)::int intended_legs,
               count(DISTINCT COALESCE(group_id,market_id::text))::int intended_units,
               count(*) FILTER (WHERE filled)::int fills,
               count(DISTINCT COALESCE(group_id,market_id::text))
                 FILTER (WHERE filled)::int filled_units,
               count(*) FILTER (
                 WHERE filled AND data_quality_grade IN ('A','B')
                   AND execution_fidelity_grade IN ('A','B')
               )::int eligible_fills,
               count(DISTINCT COALESCE(group_id,market_id::text)) FILTER (
                 WHERE filled AND data_quality_grade IN ('A','B')
                   AND execution_fidelity_grade IN ('A','B')
               )::int eligible_units,
               count(*) FILTER (WHERE filled AND pnl_2x>0)::int winning_legs_2x,
      ...
      pool.query(`
        SELECT o.strategy,l.latency_ms,
               ...
               COALESCE(sum(l.pnl_2x) FILTER (
                 WHERE l.filled AND l.data_quality_grade IN ('A','B')
  ...
  const bindings = new Map(MANIFEST.strategy_bindings
  ...
  const strategies = STRATEGIES.map((strategy) => {
    ...
    return {
      strategy,
      family: bindings.get(strategy)?.family || null,
      active: Boolean(run),
      ...
      intendedLegs: integer(result.intended_legs),
      intendedUnits: integer(result.intended_units),
      fills,
      filledUnits: integer(result.filled_units),
      eligibleFills: integer(result.eligible_fills),
      eligibleUnits: integer(result.eligible_units),
      winningLegRate2xPct: fills
        ? +(100 * integer(result.winning_legs_2x) / fills).toFixed(1) : null,
  ...
  return {
    format: 'research-v8-forward-report-v1',
    ...
    strategies,
    interpretation: [
      'A/B-quality fills and 2x-cost PnL are descriptive only.',
      'Bundle rows are legs; independentUnits groups equal-share structural legs.',
      'No profitability verdict is valid before the frozen sample, duration, split-half, clustered-confidence and multiple-testing requirements pass.',
...
function markdown(report) {
  const lines = [
    '# H64-H73 paper-forward checkpoint',
    '',
    ...
    '',
    '| Strategy | Active | Evaluations | Units | Fills | A/B units | Leg win 2x | PnL 1x | PnL 2x | Half 1 | Half 2 |',
    '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|',
  ];
  for (const row of report.strategies) {
    lines.push(`| ${row.strategy} | ${row.active ? 'YES' : 'NO'} | ${row.evaluations} | ${row.intendedUnits} | ${row.fills} | ${row.eligibleUnits} | ${row.winningLegRate2xPct == null ? '—' : `${row.winningLegRate2xPct}%`} | ${money(row.pnl1x)} | ${money(row.pnl2x)} | ${money(row.firstHalfPnl2x)} | ${money(row.secondHalfPnl2x)} |`);
  }
