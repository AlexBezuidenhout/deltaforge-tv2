...
- **Targeted materialization:** `node scripts/parquet-lake.js materialize --sources <source[,source]> --from YYYY-MM-DD --to YYYY-MM-DD --oldest --max-batches <N> --max-minutes <N>` converts only a predeclared hypothesis window. Every bounded batch still performs source SHA verification, disk-reserve preflight, immutable upload and remote checksum verification. Receipts report global and scoped pending counts separately. This is the supported way to make older evidence queryable; blindly duplicating the full 165+ GiB bronze archive is neither required nor storage-efficient.
- **Continuous projection scope:** the timer keeps strategy, resolver, cross-venue, options, structural, all-market and control decision/proof tapes nearline. High-volume CLOB, trade and options-source events stay in the verified bronze archive until a frozen backtest requests their exact source/date window. A partial automatic projection must never be interpreted as a complete raw replay.
- **Safety bound:** the continuous VPS projection is capped at 12 segments or 64 MiB compressed per run, a 24× working-expansion reserve, a 3 GiB hard memory ceiling and a hard 15 GiB free-disk floor. The first 250-segment trial was stopped before publication when it demonstrated excessive temporary expansion; a later 25-segment run peaked near 2.7 GiB and caused the tighter production envelope.
